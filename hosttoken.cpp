...
void hosttoken::addmethod(name username, name type, name detail_type, asset token, std::string details)
{
    require_auth( _self );
    ...
    methods.emplace(_self, [&](auto &m){
      ...
      m.token = token;
      m.type = type;
      m.detail_type = detail_type;
      m.details = details;
    });

}

void hosttoken::setachange(name username, uint64_t method_id, bool status)
{
    require_auth( _self );
    methods_index methods(_self, _self.value);
    ...
    std::vector<uint64_t> methods_idx;
...
void hosttoken::rmmethod(name username, uint64_t id )
{
    require_auth( _self );
...
EOSIO_DISPATCH( eosio::hosttoken, (create)(issue)(transfer)(open)(close)(retire)(createorder)(accept)(approve)(cancel)(addmethod)(rmmethod)(setachange) )
